...
test.beforeEach(async () => {
  const testcasename = test.info().title; //eslint
...
test.afterEach(async ({ page }, testInfo) => {
  ...
  // const responseStatus = testInfo.annotations.find(
  //   (a) => a.type === "responseStatus",
  // );
  // const responseBody = testInfo.annotations.find(
  //   (a) => a.type === "responseBody",
  // );
...
test("Get OLB", async ({ request }, testInfo) => {
  ...
  testInfo.annotations.push({
    type: "responseStatus",
    description: String(response.status()),
  });
  testInfo.annotations.push({
    type: "responseBody",
...
test("Create OLB", async ({ request }, testInfo) => {
  // Get the response and add to it
  const templatename = "olbcreation"; //eslint
